...
export function CaseCreatePage() {
  ...
  const cancelTo = resolveListReturnPath(location.state, '/cases', 1);
  ...
  async function submit(input: CaseFormInput): Promise<void> {
    ...
    navigate(`/cases/${created.id}`, {
      state: {
        notice: '案例已创建',
        listReturnPath: buildListPath('/cases', created.listPage),
        listFocusId: created.id,
      },
    });
  }
  ...
  return (
    <section className="event-editor-page" aria-labelledby="create-case-title">
      ...
      <CaseForm mode="create" initialContent="" onSubmit={submit} cancelTo={cancelTo} />
    </section>
  );
}
