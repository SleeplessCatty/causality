...
export function RelationListPage() {
  ...
  return (
    <LoadingState
      pending={relations.isPending}
      fetching={relations.isFetching}
      hasData={Boolean(relations.data)}
      error={semanticQueryError ? null : relations.error}
      skeleton="list"
      ...
      <section className="relation-list-page" aria-labelledby="relation-list-title">
        ...
        {expanded.isError ? (
          <div className="table-state table-state--error" role="alert">
            <strong>无法加载关系详情</strong>
            <span>请确认服务连接后重试。</span>
            <button
              className="button button--secondary"
              type="button"
              onClick={() => void expanded.refetch()}
            >
              重新加载
            </button>
          </div>
