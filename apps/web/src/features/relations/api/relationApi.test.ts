...
import { getRelations } from './relationApi';
...
describe('relationApi list search', () => {
  afterEach(() => vi.unstubAllGlobals());

  it('adds enhanced mode only to enhanced relation searches', async () => {
    ...
    await getRelations({ q: '政策收紧', page: 1, searchMode: 'enhanced' });
    await getRelations({ q: '政策收紧', page: 1 });
