...
export function EventCreatePage() {
  ...
  const cancelTo = resolveListReturnPath(location.state, '/events', 1);
  ...
  async function submit(input: EventFormInput): Promise<void> {
    ...
    navigate(`/events/${event.id}`, {
      state: {
        notice: '事件已创建',
        listReturnPath: buildListPath('/events', event.listPage),
        listFocusId: event.id,
      },
    });
  }
  ...
  return (
    <section className="event-editor-page" aria-labelledby="create-event-title">
      ...
      <EventForm mode="create" initialValue={emptyEvent} onSubmit={submit} cancelTo={cancelTo} />
    </section>
  );
}
