...
vi.mock('../../events/api/eventApi', () => ({ getEventCandidates: vi.fn() }));
...
describe('GraphEventSelector', () => {
  beforeEach(() => {
    ...
    vi.mocked(getEventCandidates).mockResolvedValue(candidates);
  });
  ...
  it('debounces non-empty searches and requests at most eight names', async () => {
    ...
    expect(getEventCandidates).not.toHaveBeenCalled();
    await act(() => vi.advanceTimersByTimeAsync(250));

    await waitFor(() => expect(getEventCandidates).toHaveBeenCalled());
    expect(vi.mocked(getEventCandidates).mock.calls[0]?.[0]).toBe('原油');
    expect(vi.mocked(getEventCandidates).mock.calls[0]?.[1]).toEqual({ limit: 8 });
  ...
  it('shows a compact candidate loading error', async () => {
    vi.mocked(getEventCandidates).mockRejectedValueOnce(new Error('network'));
    render(<GraphEventSelector value={null} onSelect={vi.fn()} />, { wrapper: Wrapper });
