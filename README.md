...
金融因果知识库。P1-03 至 P1-08 均已完成开发、自动化测试和人工验收；P1-09 已完成开发，等待人工复核。
...
- 达到当前展示限制后可按 20 → 50 → 100 节点档位扩展，100 节点后停止整体扩展；
- 可按最低置信度和最低案例数显式筛选，查询条件可由 URL 恢复；
...
图达到当前节点或关系保护上限时，状态栏会提供下一档扩展操作；依次扩展到 50 和 100 节点后不再继续整体扩展。点击“筛选”可先编辑最低置信度和最低案例数，只有点击“应用筛选”才会重新查询，并回到 20 节点档。切换中心或方向同样回到 20 节点档，但保留已应用筛选。重新查询和布局期间继续显示上一张图；失败时可直接重试。
