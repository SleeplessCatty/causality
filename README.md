...
“导入导出”页面使用 Causality 专用 CSV 数据交换格式。文件采用 UTF-8、无表头、逗号分隔，所有字段必须使用 ASCII 双引号包裹；字段内的逗号、双引号和换行按照标准 CSV 规则处理。单个文件最大 20 MiB，最多包含 50,000 条逻辑记录。

导入只追加新数据，不修改已有业务字段。文件中的有效记录会在同一事务中解析依赖并写入；无法解析的记录不会入库，也不会保存原始文件或无效内容。成功导入后可以查看批次、各类型新增与复用数量及逐条成功记录。

导出支持完整导出，以及从一个或多个原子事件出发按方向和 1–10 层深度筛选。确认框只显示本次预计导出的原子事件、因果关系和具体案例数量；确认后浏览器直接保存使用同一格式生成的 CSV 文件，服务端不保存导出文件或导出历史。
