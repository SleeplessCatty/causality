...
全链路回归覆盖：首次完整候选包含重复事件、重复案例、缺失引用和自环并被阻断；修正后的完整重提交只保留“无案例关系”和“可能跳过传递路径中的中间原子事件”存疑；对受影响关系使用 `skip` 后成功生成待确认方案；HTTP、MCP 结构化内容和普通文本警告一致；全流程未调用 `commit_knowledge_changes`。
