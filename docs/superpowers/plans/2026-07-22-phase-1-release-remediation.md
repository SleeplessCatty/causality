...
- [ ] **Step 5: 增加真实浏览器超过 20 条的回归场景**

在 `tests/e2e/cases.spec.ts` 创建或通过 API 准备 21 条关系关联到同一案例，验证详情首批为 20、点击后第 21 条可见且总数不变。
...
每个资源至少准备 `51` 条记录并验证：第一页 `50` 条、第二页剩余 `1` 条、`totalItems=51`、`totalPages=2`；搜索结果的总数只统计匹配项；案例 `relationId` 筛选总数只统计关联案例；请求超过最后一页时 Repository 把有效页钳制到最后一页。排序必须保持当前确定性规则，不因改成 OFFSET 而变化。
...
组件测试覆盖少于 `7` 页、很多页的省略号、边界禁用、当前页、点击页码、合法/非法输入跳转。三个页面测试覆盖总数信息、下一页、上一页、点击具体页码、输入任意页、搜索后回第一页、浏览器 URL 页码恢复；关系页面额外验证翻页后详情收起。E2E 为三个资源各准备至少 `51` 条可识别记录，验证第一页/第二页及总数信息；事件 E2E 额外准备 `211` 条记录，验证从第一页直接跳至第 `5` 页而不顺序请求中间页。
...
5. `使用说明`：分别简述事件、关系、案例和局部图操作；说明三个主列表每页 50 条并支持页码和直接跳转；案例上限写 100 字；图参数变化按最终实现描述。
...
用户在至少 1280×720 下复核：三个主列表的总条目数、当前/总页数、页码按钮、上一页/下一页和任意页跳转；案例详情加载第 21 条关系；关系详情加载第二批案例；图页面直接 URL 恢复、20/50/100 节点、方向和筛选；事件关键词搜索；关系一次关联多条新案例；系统状态；生产容器重启后数据持久化。
...
- README 已按正式产品结构重写，未保留阶段编号、历史测试数量、人工复核流水账、内部文档路径或未经确认的外部发布声明；
- README 的生产启动、健康检查、显式固定种子、保留数据停止、源码依赖安装、环境文件、开发 PostgreSQL 和 `pnpm dev` 均按文档命令实际验证；危险的 `docker compose down --volumes` 只核对说明，没有执行；
- `pnpm install --frozen-lockfile`、`pnpm format:check`、`pnpm lint` 和 `pnpm typecheck` 通过；
- 单元与组件测试共 280 项通过：Contracts 33、API 82、Web 165；
- PostgreSQL 集成测试 57 项通过；
- 源码 Playwright E2E 19 项通过，无跳过项，专用测试数据库已删除；
- Compose 与测试隔离合约 6 项通过；
- 图性能基准在 100,000 个事件、500,000 条关系、100,000 个案例和度数 10,000 的枢纽上完成 63 次采样；平均 9.6ms、P95 21.64ms、最大 26.84ms，低于 2,000ms 门禁；
- 隔离生产 Compose 烟雾测试 1 项通过，无跳过项；验证空库迁移、生产页面与 API、容器重启持久化及显式种子幂等性，临时项目和数据卷已清理；
