...
- [ ] Write contract tests for valid input, null confidence rejection, self-loop rejection, limits, response strictness, `caseCount: 0`, pair-check shape, and new error codes.
- [ ] Run `pnpm --filter @causality/contracts test` and confirm missing relation exports fail.
- [ ] Implement schemas and exports, then run contract tests and typecheck to green.
- [ ] Add `(updated_at desc, id desc)` and lower-description trigram indexes to the Drizzle schema.
- [ ] Extend the migration integration test first, observe missing indexes, generate migration `0002_relation_management`, inspect SQL, and rerun green.
...
- [ ] Write cursor tests for list/search states, malformed values, query mismatch, tampering, and version mismatch; confirm missing module failure.
- [ ] Implement versioned Base64URL cursors bound to normalized search text; run cursor tests green.
- [ ] Write service tests for not found, self-loop, missing events, same-direction conflict, reverse allowed, known PostgreSQL constraints, and unknown errors; confirm missing module failure.
- [ ] Implement typed `RelationServiceError` mapping and service delegation; run API unit tests green.
- [ ] Implement parameterized repository queries with stable keyset pagination, ranked unique search, pair checking, short transactions, joined event names, and `caseCount: 0`.
...
- [ ] Write Testcontainers tests for list/search/pagination, create/detail/replace, rollback, self-loop, duplicate direction, reverse direction, pair-check, missing events, not found, case count zero, indexes, and OpenAPI paths.
- [ ] Run focused integration tests and confirm relation endpoint failures.
- [ ] Register routes before the parameterized detail route, use shared Zod validation, and map errors to stable 400/404/409/500 responses.
- [ ] Register relation routes with the existing pool and preserve health/readiness construction without a pool.
...
- [ ] Write tests for navigation, loading, rows, event links, search debounce, pagination, empty/error states, inline expansion, collapse, URL state, and focused relation behavior; confirm missing pages fail.
- [ ] Implement abortable 10-second relation API helpers with shared response parsing.
- [ ] Add the relation navigation and routes without changing the established shell style.
- [ ] Implement the list and lazy detail expansion using TanStack Query caches.
- [ ] Add only relation-specific layout styles and run Web tests, typecheck, and build green.
...
- Confidence state is `number | null`; number and range controls synchronize only after user interaction.
- Pair-check blocks same direction and shows a nonblocking reverse warning.

- [ ] Write component tests for selection, module link, self-loop, pair-check states, null confidence, slider/number sync, integer bounds, preserved input, and submit locking; confirm missing components fail.
- [ ] Implement `EventSelector`, `ConfidenceInput`, and `RelationForm` with semantic labels, alerts, keyboard support, debounce, and request cancellation.
- [ ] Write route tests for create success, edit preload, full replacement, navigation to expanded list, and API failures; confirm missing pages fail.
- [ ] Implement create/edit pages, invalidate list/pair/detail caches after writes, and avoid optimistic updates.
- [ ] Run all Web tests and typecheck green.
...
- [ ] Write the failing Playwright workflow for search, create, expand, edit, duplicate blocking, reverse warning/creation, event navigation, and system readiness.
- [ ] Implement remaining testability fixes and run Playwright green against the Colima PostgreSQL database.
- [ ] Inspect list, expansion, create, and edit pages at 1280×800 and 1440×900; verify they match the established P1-03 UI baseline without a separate visual approval cycle.
- [ ] Update README and set P1-04 to `等待人工复核`.
...
- [ ] Present exact results and a concise manual checklist; do not commit implementation or enter P1-05 until the user confirms successful verification.
